...
static inline bool is_migration_disabled(const struct task_struct *p)
{
	 ...
	 *
	 * For older or backported kernels, scx_lib_init() probes the runtime
	 * behavior of the prolog and records whether it disables migration.
	 */
	if (bpf_core_field_exists(p->migration_disabled)) {
		if (p->migration_disabled == 1) {
			if (!__scx_prolog_disables_migration)
				return true;
